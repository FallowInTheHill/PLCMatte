...
class ViT(nn.Module):
	...
	def __init__(
		...
		# self.global_embed = Global_Embedding()
	...
	def forward(self, x):
		# global_x = self.global_embed(x)
		# x = torch.cat([global_x,x],dim=1)
		# trimap = x[:,-1,:,:].unsqueeze(1)
		# fg = torch.zeros(trimap.shape).to(device=x.device)
		# fg[trimap == 1] = 1
		# fg = F.interpolate(fg,scale_factor=1/16,mode='nearest')
...
class Block(nn.Module):
	...
	def forward(self, x):
		shortcut = x
		x = self.norm1(x)
		x = torch.cat([x.permute(0,3,1,2)],dim=1).permute(0,2,3,1)
		# Window partition
...
if __name__ == '__main__':
	...
	Net2 = ViT(  # Single-scale ViT backbone
							in_chans=4,
							img_size=512,
							patch_size=16,
							embed_dim=embed_dim,
	...
	Fl,Pa = profile(Net2,(x,))
	print(f'FL {Fl/1e9} PA {Pa/1e6}')
	# FL 109.697826816 G, PA 93.521664 M