...
class MattingCriterion(nn.Module):
	...
	def loss_gradient_penalty(self, sample_map ,preds, targets):
		#存在trimap全局为前景的情况
		if sample_map.max() == 0:
			return dict(loss_gradient_penalty=0)
		# preds = preds['phas']
		...
		#gradient in x
		sobel_x_kernel = torch.tensor([[[[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]]]).type(dtype=preds.type()).to(preds.device)
		delta_pred_x = F.conv2d(preds, weight=sobel_x_kernel, padding=1)
		delta_gt_x = F.conv2d(targets, weight=sobel_x_kernel, padding=1)

		#gradient in y 
		sobel_y_kernel = torch.tensor([[[[-1, -2, -1], [0, 0, 0], [1, 2, 1]]]]).type(dtype=preds.type()).to(preds.device)
		...
		# if torch.isnan(loss):
		# 	print('unknown loss_gradient_penalty')
